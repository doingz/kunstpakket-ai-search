...
const TYPE_PATTERNS = {
  ...
  Onderzetter: {
    titleKeywords: ['onderzetter', 'onderzetters', 'coaster', 'coasters'],
    contentKeywords: ['onderzetter', 'coaster'],
    categories: ['onderzetter', 'onderzetters']
  },
  Theelicht: {
    titleKeywords: ['theelicht', 'theelichthouder', 'waxinelichthouder', 'kaarshouder'],
    contentKeywords: ['theelicht', 'waxinelicht'],
    categories: ['theelicht', 'theelichten']
  },
  Spiegeldoosje: {
    titleKeywords: ['spiegeldoosje', 'spiegel doosje', 'compact mirror'],
    contentKeywords: ['spiegeldoosje', 'zakspiegeltje'],
    categories: ['spiegeldoosje']
  },
  Vaas: {
  ...
  Beeld: {
    titleKeywords: ['beeld', 'beeldje', 'beeldjes', 'sculptuur', 'sculpture', 'statue', 'bronzen', 'brons', 'figuur', 'figurine', 'justitia', 'nosybird'],
    contentKeywords: ['sculptuur', 'sculpture', 'kunsthars', 'polystone', 'fraai beeld', 'cm hoog', 'set van 2', 'met de hand gedecoreerd', 'afmetingen'],
    categories: ['beeld']
  },
  Schilderij: {
    titleKeywords: ['schilderij', 'schildering', 'painting', 'giclee', 'giclée', 'print', 'prent', 'zeefdruk'],
    contentKeywords: ['op linnen', 'op canvas', 'op doek', 'ingelijst', 'gesigneerd'],
...
/**
 * Detect product type from title, description (short desc), and category
 * Priority: title keywords > description keywords > categories
 * 
 * IMPORTANT: Uses `description` (short product summary) NOT `content` (long HTML)
 * The description is more accurate for type detection!
 */
export function detectProductType(product) {
  ...
  // Pass 2: Check description keywords (less reliable but still useful)
  for (const [type, patterns] of Object.entries(TYPE_PATTERNS)) {
    if (patterns.contentKeywords && patterns.contentKeywords.length > 0) {
      if (patterns.contentKeywords.some(kw => description.includes(kw))) {
        return type;
      }
    }
  }
